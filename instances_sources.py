...
class InstanceSource(ABC):
    @abstractmethod
    def get_instances(self, env_list: List[str], target: str) -> List[AirflowInstance]:
        pass
        ...
        for line in lines:
            ...
            if match:
                inst = AirflowInstance(
                    business_line=random.choice(["bcef", "pf", "arval", "cardif", "fortis", "BL_1", "BL_2", "BL_3", "BL_4"]),
...
class FromJsonConfig(InstanceSource):
    def get_instances(self, env_list: List[str], target: str) -> List[AirflowInstance]:
        instances_json = Variable.get(f"project_config{target}", deserialize_json=True)

        instances: List[AirflowInstance] = []

        for business_line, envs in instances_json.items():
