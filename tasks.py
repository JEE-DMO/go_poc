...
def check_instances(env_list: list[str], target: str, **context) -> str:
    source_name = Variable.get("instances_source", default_var="from_json_config")
    
    source = get_instance_source(source_name)
    
    airflow_instances = source.get_instances(env_list=env_list, target=target)
    ...
    health_data: Dict[str, Any] = {
        "statuses": [
            {
                "instance": {
                    "business_line": status.instance.business_line,
...
def send_report_email(**context):
    ...
    send_email(
       to="test@local",#Variable.get('health_check_email_recipients', default_var='airflow-team@bnp.com').split(',')
